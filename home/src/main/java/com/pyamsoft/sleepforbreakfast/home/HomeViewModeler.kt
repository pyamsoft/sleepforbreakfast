...
internal constructor(
  ...
  private fun MutableMap<DbCategory.Id, MutableSet<DbTransaction>>.categorize(transaction: DbTransaction) {
    ...
    jobs.add(
      scope.async(context = Dispatchers.Default) {
        ...
        for (t in transactions) {
          ...
          breaks.breakdown(transaction = t, today = today, startOfWeek = startOfWeek, startOfMonth = startOfMonth)
        }
  ...
  fun bind(scope: CoroutineScope) {
    listenerStatus.isNotificationListenerActive().also { f ->
      scope.launch(context = Dispatchers.Default) { f.collect { state.isNotificationListenerEnabled.value = it } }
    }
